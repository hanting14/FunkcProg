...
instance Foldable (Either' a) where
  foldr f acc (Left' a) = acc
  foldr f acc (Right' a) = (f a acc)
  foldr f acc (Both a b) = (f b acc)
...
partition [] = ([], [], [])
partition ((Left' a):xs) = (a:as, bs, cs) where
  (as, bs, cs) = partition xs
partition ((Right' a):xs) = (as, a:bs, cs) where
  (as, bs, cs) = partition xs
partition ((Both a b):xs) = (as, bs, (a,b):cs) where
  (as, bs, cs) = partition xs

zipWith' :: (Either' a b -> c) -> [a] -> [b] -> [c]
zipWith' f [] [] = []
zipWith' f (x:xs) [] = ((f (Left' x)):(zipWith' f xs []))
zipWith' f [] (x:xs) = ((f (Right' x)):(zipWith' f [] xs))
zipWith' f (x:xs) (y:ys) = ((f (Both x y)):(zipWith' f xs ys))

mapMaybeLeft :: (a -> Maybe b) -> [Either' a c] -> Maybe [Either' b c]
mapMaybeLeft _ []     = Just []
mapMaybeLeft f ((Right' x):xs) = 
    case mapMaybeLeft f xs of 
        (Nothing) -> Nothing
        Just (rest)     -> Just ((Right' x) : rest)
mapMaybeLeft f ((Left' x):xs)  =
    case f x of
        (Nothing) -> Nothing
        Just (fx) -> 
                case mapMaybeLeft f xs of
                    (Nothing) -> Nothing
                    Just (rest) -> Just ((Left' fx) : rest)
mapMaybeLeft f ((Both x y):xs) =
    case f x of
        (Nothing) -> Nothing
        Just (fx) -> 
                case mapMaybeLeft f xs of
                    (Nothing) -> Nothing
                    Just (rest) -> Just ((Both fx y) : rest)
...
instance Foldable Tree where
  ...
  foldr f acc (Node x y) = foldr f (foldr f acc y) x
...
newScope ma = do
  (env, _) <- get
  a <- ma
  modify (first (\env' -> drop (length env' - length env) env'))
  pure a
