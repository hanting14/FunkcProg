...
instance Functor Tree where
  fmap f (Node x y) = case x of
    Right a -> Node (Right (fmap f a)) (fmap f y)
    Left a ->  Node (Left (f a)) (fmap f y)
  fmap f (TriLeaf x y z) = TriLeaf (f x) (f y) (f z)

instance Foldable Tree where
  foldr f acc (Node x y) = case x of
    Right a -> let acc2 = (foldr f acc y) in foldr f acc2 a 
    Left a -> f a (foldr f acc y)
  foldr f acc (TriLeaf x y z) = f x (f y (f z acc))
...
instance Traversable Tree where
  traverse f (Node x y) = case x of
    Right a -> Node <$> (Right <$> (traverse f a)) <*> (traverse f y)
    Left a ->  Node <$> (Left <$> (f a)) <*> (traverse f y)
