...
instance Functor RoseTree where
  ...
  fmap f (Branch a x) = Branch (f a) (fmap (\y -> fmap f y) x)

instance Foldable RoseTree where
  foldr f acc (Branch a x) = f a (foldr (\t b -> foldr f b t) acc x)
...
instance Traversable RoseTree where
  traverse f (Branch a x) = Branch <$> (f a) <*> (traverse (\y -> traverse f y) x)
...
safeIndex x y
  | y >= 0 && y <= (length x)-1 = Just (x !! y)
  | otherwise = Nothing

transformWithList :: [a] -> RoseTree Int -> Maybe (RoseTree a)
transformWithList x = traverse (safeIndex x)
...
pStringLit = (char '\"' *> many (satisfy (isDigit) <|> satisfy (isLetter) <|> satisfy (==' ')) <* char' '\"')
...
pAppend :: Parser Exp
pAppend = foldl1 Append <$> sepBy1 pAdd (string' "++")
...
data Val = I Int | B Bool | S String
  deriving (Eq, Show)
type Env  = [(Name, Val)]
type Eval = State Env
...
evalSt s = case s of
  ...
  While e p -> do
    ...
    case v of
      ...
      S _ -> error "type error: expected a Bool condition in \"while\" expression"
  If e p1 p2 -> do
    v <- evalExp e
    case v of
